...
class 连线(QGraphicsPathItem):
    ...
    def __init__(self, 起点端口=None, 终点端口=None, 连线ID=None):
        ...
        self.圆弧与参数对应典 = {
                        "右向下":lambda 当前X,当前Y,R:(当前X-R, 当前Y, R * 2, R * 2, 90, -90),
                        "下向左":lambda 当前X,当前Y,R:(当前X-2*R, 当前Y-R, R * 2, R * 2, 0, -90),
                        "左向上":lambda 当前X,当前Y,R:(当前X-R, 当前Y-2*R, R * 2, R * 2, -90, -90),
                        "上向右":lambda 当前X,当前Y,R:(当前X, 当前Y-2*R, R * 2, R * 2, 180, -90),
                        "右向上":lambda 当前X,当前Y,R:(当前X-R, 当前Y-2*R, R * 2, R * 2, -90, 90),
                        "上向左":lambda 当前X,当前Y,R:(当前X-2*R, 当前Y-R, R * 2, R * 2, 0, 90),
                        "左向下":lambda 当前X,当前Y,R:(当前X-R, 当前Y, R * 2, R * 2, 90, 90),
                        "下向右":lambda 当前X,当前Y,R:(当前X, 当前Y-R, R * 2, R * 2, 180, 90)
                        }
        self.更新路径("正交折线")
    
    def 更新路径(self, 方式: str = "正交折线"):
        if not self.起点端口:
            return
        
        连线起点 = self.起点端口.scenePos() + self.起点端口.boundingRect().center()
        
        if self.终点端口:
            连线终点 = self.终点端口.scenePos() + self.终点端口.boundingRect().center()
        elif self.临时终点:
            连线终点 = self.临时终点
        else:
            return
        ...
        if 方式 == "正交折线":
            ...
            dx = 连线终点.x() - 连线起点.x() # X轴向右为正方向,原点在屏幕左上角
            dy = 连线终点.y() - 连线起点.y() # 反直觉,因为y轴向下为正方向
            
            def 添加圆角(路径对象, 方向):
                """
                用 arcTo 绘制 1/4 圆弧实现正交转角。
                ...
                - 四种方向覆盖所有 90° 正交转角
                """
                当前点 = 路径对象.currentPosition()
                当前X, 当前Y = 当前点.x(), 当前点.y()
                圆弧与参数组对应函数 = self.圆弧与参数对应典.get(方向,None)
                if 圆弧与参数组对应函数:
                    参数元组 = 圆弧与参数组对应函数(当前X,当前Y,R)
                    路径对象.arcTo(*参数元组)

            if dx >= 0 and dy >= 0:
                # 终点在起点右下方
                # 路径：起点 → 水平右 → [右下圆弧] → 垂直上 → 终点
                中点X = (起点延伸X + 终点延伸X) / 2
                
                路径.lineTo(中点X - R, 连线起点.y())
                添加圆角(路径, "右向下")            
                路径.lineTo(中点X, 连线终点.y()-R)    
                添加圆角(路径, "下向右")
                路径.lineTo(连线终点.x(), 连线终点.y())
                
            elif dx < 0 and dy >= 0:
                # 终点在起点左下方，需向下绕路
                # 路径：起点 → 水平右 → [右下圆弧] → 垂直下 → [左下圆弧] → 水平左 → [左上圆弧] → 垂直上 → 终点
                绕路Y = max(连线起点.y(), 连线终点.y()) + 绕路偏移
                路径.lineTo(起点延伸X - R, 连线起点.y())    # 到"右下"起点
                添加圆角(路径, "右向下")                # 到 (起点延伸X, 起点.y()+R)
                路径.lineTo(起点延伸X, 绕路Y - R)      # 垂直下到"左下"起点
                添加圆角(路径, "下向左")                # 到 (起点延伸X-R, 绕路Y)
                
                路径.lineTo(终点延伸X + R, 绕路Y)      # 水平左到"左上"起点
                添加圆角(路径, "左向上")                # 到 (终点延伸X, 绕路Y-R)
                
                路径.lineTo(终点延伸X, 连线终点.y()+2*R)       # 垂直上到终点Y-R
                添加圆角(路径, "上向右")
                路径.lineTo(连线终点.x(), 连线终点.y())        # 水平到终点
                
            elif dx < 0 and dy < 0:
                # 终点在起点左上方，需向上绕路
                # 路径：起点 → 水平右 → [右上圆弧] → 垂直上 → [左上圆弧] → 水平左 → [左下圆弧] → 垂直下 → 终点
                绕路Y = min(连线起点.y(), 连线终点.y()) - 绕路偏移
                
                路径.lineTo(起点延伸X - R, 连线起点.y())    # 到"右上"起点
                添加圆角(路径, "右向上")                # 到 (起点延伸X, 起点.y()-R)
                
                路径.lineTo(起点延伸X, 绕路Y + R)      # 垂直上到"左上"起点
                添加圆角(路径, "上向左")                # 到 (起点延伸X-R, 绕路Y)
                
                路径.lineTo(终点延伸X + R, 绕路Y)      # 水平左到"左下"起点
                添加圆角(路径, "左向下")                # 到 (终点延伸X, 绕路Y+R)
                
                路径.lineTo(终点延伸X, 连线终点.y()-R)       # 垂直下到终点Y
                添加圆角(路径, "下向右")
                路径.lineTo(连线终点.x(), 连线终点.y())        # 水平到终点
                
            else:
                # 终点在起点右上方
                # 路径：起点 → 水平右 → [右上圆弧] → 垂直上 → 终点
                中点X = (起点延伸X + 终点延伸X) / 2
                
                路径.lineTo(中点X - R, 连线起点.y())      # 到"右上"起点
                添加圆角(路径, "右向上")                # 到 (中点X, 起点.y()-R)
                路径.lineTo(中点X, 连线终点.y()+2*R)           # 垂直向上
                添加圆角(路径, "上向右")
                路径.lineTo(连线终点.x(), 连线终点.y())        # 水平到终点
            ...
            dx = abs(连线终点.x() - 连线起点.x())
            控制距离 = max(dx * 0.5, 50)
            
            控制1 = QPointF(连线起点.x() + 控制距离, 连线起点.y())
            控制2 = QPointF(连线终点.x() - 控制距离, 连线终点.y())
            
            路径.cubicTo(控制1, 控制2, 连线终点)
            self.setPath(路径)
            return
    ...
    def 导出字典(self) -> dict:
        ...
        for 连线项 in self._连线:
            if 连线项.起点端口 and 连线项.终点端口:
                ...
                终点节点 = 连线项.终点端口.parentItem()
                
                数据["连线"].append({
                    "id": 连线项.连线ID,
                    "起点节点": 起点节点.节点ID if 起点节点 else None,
                    "起点端口": 连线项.起点端口.名称,
                    "终点节点": 终点节点.节点ID if 终点节点 else None,
                    "终点端口": 连线项.终点端口.名称
                })
        
        return 数据
    
    def 导入字典(self, 数据: dict):
        """从字典导入节点和连线，复用 _创建有效连线 确保方向正确"""
        # 清空当前画布
        self.清空画布()
        节点映射 = {}  # 用于映射旧ID到新节点对象
        
        # ========== 第一阶段：创建所有节点 ==========
        for 节点数据 in 数据.get("节点", []):
            原始节点ID = 节点数据.get("id")
            节点类型 = 节点数据.get("类型")
            节点名称 = 节点数据.get("名称", 节点类型)
            位置数据 = 节点数据.get("位置", {"x": 0, "y": 0})
            场景位置 = QPointF(位置数据.get("x", 0), 位置数据.get("y", 0))
            
            新节点 = None
            
            try:
                if 节点类型 == "开始":
                    新节点 = 开始节点(原始节点ID, 节点名称)
                    新节点.setPos(场景位置)
                    self.场景.addItem(新节点)
                    self._节点[原始节点ID] = 新节点
                    状态.注册节点(原始节点ID, 新节点)
                    ...
                    if "输出数据" in 节点数据:
                        新节点.输出数据 = 节点数据["输出数据"]
                        新节点.数据标签.setPlainText(新节点.输出数据 or "点击输入数据...")
                        
                elif 节点类型 == "插件":
                    插件名 = 节点数据.get("插件名", "未知")
                    新节点 = 插件节点(原始节点ID, 插件名, 节点名称)
                    新节点.setPos(场景位置)
                    self.场景.addItem(新节点)
                    self._节点[原始节点ID] = 新节点
                    状态.注册节点(原始节点ID, 新节点)
                    ...
                    if "判断条件" in 节点数据:
                        新节点.判断条件 = 节点数据["判断条件"]
                        新节点.条件标签.setPlainText(新节点.判断条件 or "点击设置条件...")
                        
                elif 节点类型 == "展示":
                    新节点 = 展示节点(原始节点ID, 节点名称)
                    新节点.setPos(场景位置)
                    self.场景.addItem(新节点)
                    self._节点[原始节点ID] = 新节点
                    状态.注册节点(原始节点ID, 新节点)
                    ...
                    if "历史数据" in 节点数据:
                        for 历史项 in 节点数据["历史数据"]:
                            ...
                            新节点.历史数据.append(数据项)
                            if 数据项["类型"] == "接收":
                                新节点.历史接收数据.append(数据项["内容"])
                            else:
                                新节点.历史发送数据.append(数据项["内容"])
                        新节点._更新计数显示()
                    if "模板数据" in 节点数据:
                        新节点.模板数据 = 节点数据["模板数据"].copy()
                # 处理端口配置
                if 新节点:
                    保存的输入端口 = 节点数据.get("输入端口", [])
                    保存的输出端口 = 节点数据.get("输出端口", [])
                    # 清除默认端口
                    for 端口名称, 端口对象 in list(新节点.输入端口.items()):
                        端口对象.连线列表.clear()
                        self.场景.removeItem(端口对象)
                    for 端口名称, 端口对象 in list(新节点.输出端口.items()):
                        端口对象.连线列表.clear()
                        self.场景.removeItem(端口对象)
                    新节点.输入端口.clear()
                    新节点.输出端口.clear()
                    ...
                    for 端口信息 in 保存的输出端口:
                        ...
                        if 端口名:
                            新节点.添加输出端口(端口名)
                    新节点._更新名称()
                    self.节点创建信号.emit(原始节点ID, 新节点)
                    信号.节点创建.emit(原始节点ID, 新节点)
                    节点映射[原始节点ID] = 新节点
                    日志.信息(f"导入节点: {原始节点ID} ({节点名称})")
            except Exception as e:
                日志.错误(f"导入节点 {原始节点ID} 失败: {e}")
                import traceback
                日志.调试(traceback.format_exc())
                continue
        # ========== 第二阶段：创建所有连线（复用公共方法）==========
        for 连线数据 in 数据.get("连线", []):
            原始起点节点ID = 连线数据.get("起点节点")
            起点端口名 = 连线数据.get("起点端口")
            原始终点节点ID = 连线数据.get("终点节点")
            终点端口名 = 连线数据.get("终点端口")
            连线ID = 连线数据.get("id")
            # 获取节点
            起点节点 = 节点映射.get(原始起点节点ID)
            终点节点 = 节点映射.get(原始终点节点ID)
            if not 起点节点 or not 终点节点:
                日志.警告(f"连线 {连线ID} 的节点不存在: {原始起点节点ID} -> {原始终点节点ID}，跳过")
                continue
            # 获取端口（不指定类型，让公共方法自动处理）
            起点端口 = 起点节点.取端口(起点端口名)
            终点端口 = 终点节点.取端口(终点端口名)
            if not 起点端口 or not 终点端口:
                日志.警告(f"连线 {连线ID} 的端口不存在: {原始起点节点ID}.{起点端口名} 或 {原始终点节点ID}.{终点端口名}，跳过")
                continue
            # 使用公共方法创建连线（启用自动纠正，跳过逻辑验证以兼容旧文件）
            新连线 = self._创建有效连线(
                原始起点端口=起点端口,
                原始终点端口=终点端口,
                连线ID=连线ID,
                自动纠正方向=True,      # 关键：启用自动纠正
                跳过逻辑验证=True       # 导入时跳过，确保兼容性
            )
            # 失败日志已在公共方法中记录        
        日志.信息(f"导入完成: {len(节点映射)} 个节点, {len(self._连线)} 条连线")
