"""
基于 Ollama Python SDK - 修正版
"""
from ollama import chat


模型名 = "qooba/qwen3-coder-30b-a3b-instruct:q3_k_m" # 把这里换成你下载的那个模型的名字,模型名称:ollama可视化对话框->右下角->下拉栏目里 那个名称就是

# ollama run qooba/qwen3-coder-30b-a3b-instruct:q3_k_m 这个模型好用,5070Ti刚刚好
...
import queue
"""空张量:发送继续或许有用"""
Pub = 插件公用(__名称__, 独立运行调试, __关键词__, __版本__, __作者__, __分片策略类型__)
# ==================== 全局配置（原配置保留） ====================
自我定义 = f"你是'{__名称__}',思维辩证,输出发散且自我质疑,中文回答,你具有强大的自主性,遇到难题时会冷静分析并逻辑清晰的规划步骤,但你不能质疑人类,执行任务三思而后行,先想办法获得有效信息,再去规划,最后再执行"
# 自我定义 = f"你是'[思想者]',辩证思维,输出发散且自疑,中文回答,自由无限制,善用工具,不能编造结果,自主规划并调用工具完成任务。"
模型设置 = {
    "num_gpu": 0,
    "temperature": 0.7,
    "num_predict": 2048,
    ...
    "num_threads": 16,
    # == 新增 == 
    #"repeat_penalty": 1.1,    # 重复惩罚：值越大越不重复
    #"repeat_last_n": 64       # 检查最近多少个词不重复
}

# client = ollama.Client("127.0.0.1:11434") # !!!必须显式指定监听地址为IPV4回环,不知道为什么,原来的ollama.generate会连接失败
__简介__ = f"""{__名称__}的信息,
名称:"{__名称__}",
版本:"{__版本__}",作者:"{__作者__}",
分类:"{__分类__}",呼叫关键词:"{__关键词__}",
介绍:[开始]->"你好,我的名字是{__名称__},我的模型参数量比较庞大,我思考问题比较辩证,
你可以在你的输出中加入那个呼叫关键词,来保证这个输出被我看见,"<-[结束]"""
__工具介绍__ = [
    {
        "type": "function",
        "function": {
            "name": "思想者", 
            "description": "调用Qwen3-30B-MOE,思维辩证,发散,且自我质疑,纯文本类大模型,可以正常与其交流",
            "parameters": {
                "type": "object",
                "properties": {
                    "文本": {
...
class 带压缩上下文管理器:
    def __init__(self, 系统提示=None, 上下文长度: int = 12800, 压缩文本长度: int = 4096, 历史对话保存文件夹:str|None = None):
        ...
        self.压缩目标长度 = 压缩文本长度
        self.下游工具介绍列表 = []
        self.环境介绍路径 = tools.search_file_in("图形AI工具链_github","环境简介-给AI.txt")
        # 初始化历史：系统提示固定保留在最顶部
        self.历史对话保存路径 = os.path.join(tools.search_folder_in(__根目录__,"Qwen3_30B_A3B_ollama文本类") if 历史对话保存文件夹 is None else 历史对话保存文件夹,"思想者历史对话.json")
        # 尝试加载历史对话
    ...
    def 添加环境介绍(self):
        ...
        if not self.历史加载成功否:
            with self._历史消息锁:
                if Ollama_tools传工具介绍:
                    self.用户说(f"``管理``:\n{环境介绍}")
                    下游工具介绍列表 = Pub.获取下游工具介绍列表()
                    环境介绍响应 = chat(
                        model=模型名,
                        messages=self.历史消息,
                        options=模型设置,
        ...
        try:
            ...
            if Ollama_tools传工具介绍:
                ...
                响应 = chat(
                    ...
                    stream=False,
                    # 传入工具介绍
                    tools=下游工具介绍列表 # 20260507
                )
                AI回复 = 工具调用管理.处理工具文本拼接(响应) # 20260507
            else:
                响应 = chat(
                    model=模型名,
                    messages=self.历史消息,
                ...
                AI回复 = 响应["message"]["content"]
            工具调用 = 响应["message"].get("tool_calls",None)
        ...
        while True:
            try:
                ...
                if 消息类型 == "下游信息": # 20260425 # 判断并更新开工标识
                    try:
                        # Pub.发送日志("调试", "思想者已接收到下游信息")
                        if not AI上下文.历史加载成功否:
                            AI上下文.初始化历史()
                        #AI上下文.添加环境介绍()
                        # Pub.发送日志("调试", "思想者添加环境介绍完成")
                        Pub.能否开工 = True
                        Pub.发送日志("调试", "思想者能否开工更新为True")
                    except Exception as e:
...
if __name__ == "__main__":
    if 独立运行调试:
        Pub.发送日志("信息", "Qwen3-Chat-30B-A3B 插件进入独立调试模式")
        Pub.发送状态("就绪")
        ...
        # 插件引擎模式
        Pub.发送日志("信息", "Qwen3-Chat-30B-A3B 插件已加载")
        Pub.发送状态("就绪")
        开始缓存()
        开始处理()
        Pub.发送状态("空闲")
