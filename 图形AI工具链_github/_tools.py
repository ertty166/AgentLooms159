...
def get_now_path(type: str = "folder") -> Optional[str]:
    """获取当前文件或所在文件夹的路径,传入folder or file"""
    try:
        if type == "folder":
...
def search_folder_in(upto: int|str, foldername: Optional[str] = None, prompt: int = 0, build=False, BuildPath=None) -> Optional[str]:
    """向上查找指定文件夹"""
...
- 当前路径：{s_root_path}""")
    ...
    try:
        ...
        if build:
            # 在目标位置创建文件夹 或 在根目录创建文件夹
            depend_dir = os.path.join(s_root_path if not BuildPath else BuildPath, foldername)
...
def search_file_in(upto: int|str = 1, filename: Optional[str] = None, prompt: int = 0, build=False, BuildPath=None) -> Optional[str]:
    ...
    try:
        ...
        if build:
            if not BuildPath:
                ...
                # 在目标路径下创建文件
                file_path = os.path.join(BuildPath, filename)
                # 确保目录存在（虽然 s_root_path 应该已存在）
                os.makedirs(os.path.dirname(file_path) if os.path.dirname(filename) else s_root_path, exist_ok=True)
                with open(file_path, "w", encoding="utf-8") as f:
                    pass  # 创建空文件
